...
class ComplaintImporterTestCase(TestCase):
    def setUp(self):
        self.header = ['allegation_uid', 'uid', 'tracking_id',
                       ...
                       'allegation', 'citizen', 'disposition', 'charges', 'complainant_name',
        ...
        self.complaint1_data = ['complaint-uid1-allegation-uid1-charge-uid1', 'officer-uid1', '2018-018',
                                ...
                                'black female', 'sustained',
                                '3:22 violation of known laws - 56 violations', 'Chief Najolia ', 'internal', 'female',
                                'black', 'hold in abeyance', '2016', '', 'discourtesy', 'supervisor-uid1',
                                'assistant chief', 'HP-50', 'P10382', 'patrol 1st district',
        ...
        self.complaint3_data = ['complaint-uid1', 'officer-uid2', '2015-2', 'complete', '', '', '',
                                ...
                                'hispanic', '', '2020', 'baton-rouge-pd', 'discourtesy', '', '', 'HP-50', '',
                                'off-duty detail', '', '', '']
    ...
    def test_process_successfully(self):
        ...
        self.complaint_importer.old_column_mappings = {column: self.header.index(column) for column in self.header}
    ...
    def test_process_successfully_with_column_changed(self):
        ...
        self.complaint_importer.old_column_mappings = {column: old_columns.index(column) for column in old_columns}
        self.complaint_importer.column_mappings = {column: self.header.index(column) for column in self.header}
        ...
        for complaint_data in expected_complaints_data:
            ...
            for attr in field_attrs:
                assert getattr(complaint, attr) == (complaint_data[self.complaint_importer.column_mappings[attr]]
                                                    if complaint_data[
                    self.complaint_importer.column_mappings[attr]] else None)

            assert list(complaint.departments.values_list('id', flat=True)) == complaint_data[
                self.complaint_importer.column_mappings['department_ids']]
            assert list(complaint.officers.values_list('id', flat=True)) == complaint_data[
