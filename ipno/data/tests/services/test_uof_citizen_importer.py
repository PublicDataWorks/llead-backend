...
class UofImporterTestCase(TestCase):
    ...
    def test_process_successfully(self):
        ...
        UseOfForceCitizenFactory(uof_citizen_uid='uof_citizen_uid1', uof_uid='uof-uid1', use_of_force=use_of_force_1)
        UseOfForceCitizenFactory(uof_citizen_uid='uof_citizen_uid2', uof_uid='uof-uid2', use_of_force=use_of_force_2)
        UseOfForceCitizenFactory(uof_citizen_uid='uof_citizen_uid3', uof_uid='uof-uid3', use_of_force=use_of_force_3)
        ...
        uof_citizen_importer.old_column_mappings = {column: self.header.index(column) for column in self.header}
        uof_citizen_importer.column_mappings = {column: self.header.index(column) for column in self.header}
        ...
        for uof_citizen_data in expected_uof_citizens_data:
            uof_citizen = UseOfForceCitizen.objects.filter(
                uof_citizen_uid=uof_citizen_data[check_columns_mappings['uof_citizen_uid']],
            ).first()
    ...
    def test_process_successfully_with_columns_changed(self):
        ...
        uof_citizen_importer.old_column_mappings = {column: old_columns.index(column) for column in old_columns}
        uof_citizen_importer.column_mappings = {column: self.header.index(column) for column in self.header}
