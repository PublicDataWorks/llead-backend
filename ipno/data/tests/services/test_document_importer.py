...
class DocumentImporterTestCase(TestCase):
    def setUp(self):
        ...
        self.document1_data = [
            ...
            "e8a785ca3624bce9fe76a630fd6dbf07ab194202ef135480c76d9dbee79ab8ff",
            "2018",
            "6",
            "14",
            "scraped",
            "1",
            "Joseph Jones, Docket No. 17-",
            "officer-uid-1",
            "new-orleans-pd",
            cleandoc(
                """
        ...
        self.document2_data = [
            ...
            "2c668256378a491fd2f2812fcd4fc0f22af292b66f3f63ce6070321c57497f5a",
            "2018",
            "12",
            "18",
            "scraped",
            "1",
            "Officer Terry Guillory, Docket 17-201",
            "officer-uid-2",
            "",
            "",
            "unknown",
        ...
        self.document3_data = [
            ...
            "affc812dbf419a261ba5edd110c7abef90a0a3e7ee0ec285b1e90cba2f7680a7",
            "1999",
            "9",
            "30",
            "scraped",
    ...
    def test_process_fail_dueto_dropbox_get_file_error(self):
        ...
        document_importer.old_column_mappings = {
            column: self.header.index(column) for column in self.header
        }
        document_importer.column_mappings = {
            column: self.header.index(column) for column in self.header
        }
    ...
    def test_upload_file_success(self):
        ...
        document_importer = DocumentImporter()
        document_importer.gs = MagicMock(
            upload_file_from_string=mock_upload_file_from_string
        )

        download_url = document_importer.upload_file(
    ...
    def test_process_successfully(self, generate_from_blob_mock, get_mock):
        ...
        for document_data in expected_documents_data:
            document = Document.objects.filter(
                docid=document_data[check_columns_mappings["docid"]]
                if document_data[check_columns_mappings["docid"]]
                else None,
                hrg_no=document_data[check_columns_mappings["hrg_no"]]
                if document_data[check_columns_mappings["hrg_no"]]
                else None,
                matched_uid=document_data[check_columns_mappings["matched_uid"]]
                if document_data[check_columns_mappings["matched_uid"]]
                else None,
                agency=slugify(document_data[check_columns_mappings["agency"]])
                if document_data[check_columns_mappings["agency"]]
                else None,
            ).first()
            assert document
            field_attrs = [
            ...
            for attr in field_attrs:
                assert getattr(document, attr) == (
                    document_data[check_columns_mappings[attr]]
                    if document_data[check_columns_mappings[attr]]
                    else None
                )

            for attr in integer_field_attrs:
                assert getattr(document, attr) == (
                    int(document_data[check_columns_mappings[attr]])
                    if document_data[check_columns_mappings[attr]]
                    else None
                )
            ...
            ocr_text = (
                f'ocr text for {document_data[check_columns_mappings["txt_db_id"]]}'
            )
            document_text_content = (
                document_data[check_columns_mappings["hrg_text"]]
                if document_data[check_columns_mappings["hrg_text"]]
                else ocr_text
            )
            ...
            assert document.preview_image_url == document_url.replace(
                ".pdf", "-preview.jpeg"
            ).replace(".PDF", "-preview.jpeg")

            assert (
                list(document.departments.values_list("id", flat=True))
                == document_data[check_columns_mappings["department_ids"]]
            )
            assert (
                list(document.officers.values_list("id", flat=True))
                == document_data[check_columns_mappings["officer_ids"]]
            )
    ...
    @patch(
        "data.services.document_importer.generate_from_blob",
        return_value="preview_image_blob",
    )
    def test_generate_preview_image_success(self, _):
        ...
        preview_image_url = document_importer.generate_preview_image(
            image_blob, upload_url
        )
    ...
    def test_handle_pdf_file_process_success(self, get_mock):
        ...
        document_importer = DocumentImporter()
        document_importer.ds = mock_dropbox
        document_importer.generate_preview_image = Mock(
            return_value="preview_image_blob"
        )
        ...
        assert uploaded_url == {
            "document_url": (
                "https://storage.googleapis.com/llead-documents-test/pdf_db_path"
            ),
