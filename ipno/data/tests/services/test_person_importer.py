...
class PersonImporterTestCase(TestCase):
    def setUp(self):
        ...
        self.person1_data = ['1', '000526fa05739e97b61343513a92dbc7', '000526fa05739e97b61343513a92dbc7']
        self.person2_data = [
            '2',
        ...
        self.person4_data = ['3', '0007f3f6802b9fa2d4a23edc00b', '0007f3f6802b9fa2d4a23edc00b']
    ...
    def test_process_successfully(self):
        ...
        assert results.first().person_id == expected_person1_data[check_columns['person_id']]
        assert results.first().canonical_officer.uid == expected_person1_data[check_columns['canonical_uid']]
        assert not results.first().all_complaints_count
        assert expected_person1_data[check_columns['uids']] in results.first().officers.values_list('uid', flat=True)

        expected_person3_data = self.person3_data.copy()
        assert results.last().person_id == expected_person3_data[check_columns['person_id']]
        assert results.last().canonical_officer.uid == expected_person3_data[check_columns['canonical_uid']]
        ...
        assert expected_person3_data[check_columns['uids']].split(',')[0] in expected_uids_list
        assert expected_person3_data[check_columns['uids']].split(',')[1] not in expected_uids_list
