...
class UofCitizenImporter(BaseImporter):
    ...
    def get_uof_citizen_mappings(self):
        return {
            (
                use_of_force_citizen.uof_citizen_uid,
                use_of_force_citizen.uof_uid,
            ): use_of_force_citizen.id
            for use_of_force_citizen in UseOfForceCitizen.objects.only(
                "id", "uof_citizen_uid", "uof_uid"
            )
        }
