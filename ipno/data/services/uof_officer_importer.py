...
class UofOfficerImporter(BaseImporter):
    ...
    def import_data(self, data):
        ...
        return self.bulk_import(UseOfForceOfficer, self.new_uof_officers_attrs, self.update_uof_officers_attrs, self.delete_uof_officer_ids)

    def get_uof_officer_mappings(self):
        return {
            (use_of_force_officer.uof_uid, use_of_force_officer.uid): use_of_force_officer.id
            for use_of_force_officer in UseOfForceOfficer.objects.only('id', 'uof_uid', 'uid')
        }
