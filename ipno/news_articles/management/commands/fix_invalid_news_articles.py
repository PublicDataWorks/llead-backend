...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        inv_news = NewsArticle.objects.select_related('source').filter(guid__startswith='http', url__contains='_http')
        ...
        for news in tqdm(inv_news):
            ...
            file_name = f'{news.published_date.strftime("%Y-%m-%d")}_{slugify(news.title)}.pdf'
            new_file_path = f'{NEWS_ARTICLE_CLOUD_SPACES}/{news.source.source_name}/{file_name}'
