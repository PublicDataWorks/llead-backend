...
class FixInvalidNewsArticlesTestCase(TestCase):
    @patch('news_articles.management.commands.fix_invalid_news_articles.NEWS_ARTICLE_CLOUD_SPACES', 'news_articles')
    @patch('news_articles.management.commands.fix_invalid_news_articles.GoogleCloudService')
    def test_call_command(self, mock_google_cloud_service):
        ...
        old_file_path_1 = f'news_articles/{source.source_name}/2020-04-05_{article_1.guid}.pdf'
        old_file_path_2 = f'news_articles/{source.source_name}/2020-04-05_{article_2.guid}.pdf'

        mock_gcs_return.is_object_exists.assert_has_calls([
            call(old_file_path_1),
            call(old_file_path_2)
        ])
