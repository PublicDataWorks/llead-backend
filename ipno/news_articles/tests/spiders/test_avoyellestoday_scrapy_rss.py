...
from news_articles.constants import AVOYELLESTODAY_SOURCE
from news_articles.factories import NewsArticleSourceFactory
from news_articles.models import NewsArticle, CrawledPost
from news_articles.spiders import AvoyellesTodayScrapyRssSpider
from officers.factories import OfficerFactory
from utils.constants import FILE_TYPES


class AvoyellesTodayScrapyRssSpiderTestCase(TestCase):
    @patch('news_articles.spiders.base_scrapy_rss.GoogleCloudService')
    def setUp(self, mock_gcloud_service):
        NewsArticleSourceFactory(source_name=AVOYELLESTODAY_SOURCE)
        self.spider = AvoyellesTodayScrapyRssSpider()

    def test_parse_item_path(self):
        with open(join(dirname(__file__), 'files', 'avoyellestoday.xml'), 'r') as f:
            file_content = f.read()
        feed_url = 'https://www.avoyellestoday.com/rss.xml'
        ...
        assert self.articles[0]['title'].strip() == 'Trick-R-Treat times in Avoyelles'

        assert self.articles[0]['link'].strip() == 'https://www.avoyellestoday.com/news/trick-r-treat-times-avoyelles'

        assert self.articles[0]['guid'].strip() == '201070 at https://www.avoyellestoday.com'

        assert self.articles[0]['published_date'].strip() == 'Tue, 26 Oct 2021 19:05:37 +0000'

    @patch('news_articles.spiders.avoyellestoday_scrapy_rss.ItemLoader')
    @patch('news_articles.spiders.avoyellestoday_scrapy_rss.RSSItem')
    ...
    @patch('news_articles.spiders.avoyellestoday_scrapy_rss.ArticlePdfCreator')
    def test_parse_article(self, mock_article_pdf_creator):
        ...
        assert new_article.source.source_name == 'avoyellestoday'
        ...
        assert crawled_post.source.source_name == 'avoyellestoday'
