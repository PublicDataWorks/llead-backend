...
class KlaxRssSpiderTestCase(TestCase):
    ...
    def test_parse_item_path(self):
        ...
        assert (
            ...
            == "Jena man arrested for second degree murder"
        )

        assert (
            self.articles[0]["link"].strip()
            == "http://klax-tv.com/jena-man-arrested-for-second-degree-murder"
    ...
    def test_call_parse_items_call_params(
        self, mock_selector, mock_rss_item, mock_item_loader
    ):
        ...
        mock_selector.return_value = Mock(
            xpath=mock_xpath, remove_namespaces=mock_remove_namespaces
        )
