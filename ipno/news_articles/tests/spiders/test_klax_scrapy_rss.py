...
from news_articles.spiders import KlaxScrapyRssSpider
from officers.factories import OfficerFactory
from utils.constants import FILE_TYPES


class KlaxRssSpiderTestCase(TestCase):
    @patch('news_articles.spiders.base_scrapy_rss.GoogleCloudService')
    def setUp(self, mock_gcloud_service):
        NewsArticleSourceFactory(source_name=KLAX_SOURCE)
        self.spider = KlaxScrapyRssSpider()

    def test_parse_item_path(self):
        with open(join(dirname(__file__), 'files', 'klax.xml'), 'r') as f:
            file_content = f.read()
        feed_url = 'http://klax-tv.com/feed/'
        ...
        assert self.articles[0]['title'].strip() == 'Jena man arrested for second degree murder'

        assert self.articles[0]['link'].strip() == 'http://klax-tv.com/jena-man-arrested-for-second-degree-murder' \
                                                   '/?utm_source=rss&utm_medium=rss&utm_campaign=jena-man' \
                                                   '-arrested-for-second-degree-murder'

        assert self.articles[0]['guid'].strip() == 'http://klax-tv.com/?p=76513'

        assert self.articles[0]['published_date'].strip() == 'Sun, 17 Oct 2021 14:12:16 +0000'
    ...
    @patch('news_articles.spiders.klax_scrapy_rss.ArticlePdfCreator')
    def test_parse_article(self, mock_article_pdf_creator):
        ...
        mock_css.assert_called_with(".entry-content.clearfix>:not(meta):not(img):not(div),div[dir=\"auto\"]")
        ...
        assert new_article.source.source_name == 'klax'
        ...
        assert crawled_post.source.source_name == 'klax'
    ...
    @patch('news_articles.spiders.klax_scrapy_rss.ArticlePdfCreator')
    def test_parse_article_with_empty_text_content(self, mock_article_pdf_creator):
        officer = OfficerFactory()
        ...
        self.spider.parse_paragraphs = Mock()
        self.spider.get_upload_pdf_location = Mock()
        self.spider.upload_file_to_gcloud = Mock()
        ...
        self.spider.parse_paragraphs.assert_not_called()
        self.spider.get_upload_pdf_location.assert_not_called()
