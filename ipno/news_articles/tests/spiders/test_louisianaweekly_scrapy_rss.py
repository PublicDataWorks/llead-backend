...
class LouisianaWeeklyScrapyRssSpiderTestCase(TestCase):
    ...
    def test_parse_item_path(self):
        ...
        assert (
            ...
            == "FBI: At least four Black females were murdered each day in 2020"
        )

        assert (
            self.articles[0]["link"].strip()
            == "http://www.louisianaweekly.com/fbi-at-least-four-black-females"
            "-were-murdered-each-day-in-2020/"
        )

        assert (
            self.articles[0]["guid"].strip()
            == "http://www.louisianaweekly.com/?p=61476"
        )

        assert (
            self.articles[0]["published_date"].strip()
            == "Mon, 18 Oct 2021 22:14:18 +0000"
        )
    ...
    def test_create_article_with_author_name_in_content(self, mock_article_pdf_creator):
        html_content = (
            ...
            " Writer</em></p><p>The uniformed crime reporting statistics revealed that"
            " .. </p> "
        )
