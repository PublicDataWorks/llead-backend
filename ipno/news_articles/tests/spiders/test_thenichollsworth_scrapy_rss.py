...
class NichollsWorthScrapyRssSpiderTestCase(TestCase):
    ...
    def test_parse_item_path(self):
        ...
        assert self.articles[0]['title'].strip() == 'Colonel esports creating inclusive space for gamers'

        assert self.articles[0]['link'].strip() == 'https://thenichollsworth.com/7010279/sports/colonel-esports' \
                                                   '-creating-inclusive-space-for-gamers/'

        assert self.articles[0]['guid'].strip() == 'https://thenichollsworth.com/?p=7010279'

        assert self.articles[0]['published_date'].strip() == 'Fri, 22 Oct 2021 11:01:57 +0000'
