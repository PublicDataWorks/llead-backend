...
class LoyolaMaroonScrapyRssSpiderTestCase(TestCase):
    ...
    def test_parse_item_path(self):
        ...
        assert (
            ...
            == "Students visit local abandoned buildings"
        )

        assert (
            self.articles[0]["link"].strip()
            == "https://loyolamaroon.com/10032803/life-times/students-visit-local"
            "-abandoned-buildings/"
        )

        assert (
            self.articles[0]["guid"].strip() == "https://loyolamaroon.com/?p=10032803"
        )

        assert (
            self.articles[0]["published_date"].strip()
            == "Thu, 21 Oct 2021 21:55:49 +0000"
        )
