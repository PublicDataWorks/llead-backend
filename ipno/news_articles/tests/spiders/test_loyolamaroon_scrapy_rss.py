...
from news_articles.constants import LOYOLAMAROON_SOURCE
from news_articles.factories import NewsArticleSourceFactory
from news_articles.models import NewsArticle, CrawledPost
from news_articles.spiders import LoyolaMaroonScrapyRssSpider
from officers.factories import OfficerFactory
from utils.constants import FILE_TYPES


class LoyolaMaroonScrapyRssSpiderTestCase(TestCase):
    @patch('news_articles.spiders.base_scrapy_rss.GoogleCloudService')
    def setUp(self, mock_gcloud_service):
        NewsArticleSourceFactory(source_name=LOYOLAMAROON_SOURCE)
        self.spider = LoyolaMaroonScrapyRssSpider()

    def test_parse_item_path(self):
        with open(join(dirname(__file__), 'files', 'loyolamaroon.xml'), 'r') as f:
            file_content = f.read()
        feed_url = 'https://loyolamaroon.com/feed/'
        ...
        assert self.articles[0]['title'].strip() == 'Students visit local abandoned buildings'

        assert self.articles[0]['link'].strip() == 'https://loyolamaroon.com/10032803/life-times/students-visit-local' \
                                                   '-abandoned-buildings/'

        assert self.articles[0]['guid'].strip() == 'https://loyolamaroon.com/?p=10032803'

        assert self.articles[0]['published_date'].strip() == 'Thu, 21 Oct 2021 21:55:49 +0000'

        assert self.articles[0]['content'][:20] == '<p>Young artists at '
        assert self.articles[0]['content'][-20:] == 'The Maroon</a>.</p>\n'

    @patch('news_articles.spiders.loyolamaroon_scrapy_rss.ItemLoader')
    @patch('news_articles.spiders.loyolamaroon_scrapy_rss.RSSItem')
    ...
    @patch('news_articles.spiders.loyolamaroon_scrapy_rss.ArticlePdfCreator')
    def test_create_article(self, mock_article_pdf_creator):
        ...
        assert new_article.source.source_name == 'loyolamaroon'
        ...
        assert crawled_post.source.source_name == 'loyolamaroon'
