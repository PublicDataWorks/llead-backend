...
class ConcordiaSentinelScrapyRssSpiderTestCase(TestCase):
    ...
    def test_parse_item_path(self):
        with open(join(dirname(__file__), 'files', 'concordiasentinelhannapub.xml'), 'r') as f:
            file_content = f.read()
        ...
        assert self.articles[0]['link'].strip() == 'https://www.hannapub.com/concordiasentinel/obituaries---oct-20' \
                                                   '-2021/article_9e19ae48-31cd-11ec-8321-77ec86cfa148.html'

        assert self.articles[0]['guid'].strip() == 'http://www.hannapub.com/tncms/asset/editorial/9e19ae48-31cd-11ec' \
                                                   '-8321-77ec86cfa148'
    ...
    @patch('news_articles.spiders.concordiasentinelhannapub_scrapy_rss.ArticlePdfCreator')
