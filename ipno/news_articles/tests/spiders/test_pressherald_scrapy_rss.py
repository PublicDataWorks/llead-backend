...
from news_articles.constants import PRESSHERALD_SOURCE
from news_articles.factories import NewsArticleSourceFactory
from news_articles.models import NewsArticle, CrawledPost
from news_articles.spiders import MindenPressHeraldScrapyRssSpider
from officers.factories import OfficerFactory
from utils.constants import FILE_TYPES


class MindenPressHeraldScrapyRssSpiderTestCase(TestCase):
    @patch('news_articles.spiders.base_scrapy_rss.GoogleCloudService')
    def setUp(self, mock_gcloud_service):
        NewsArticleSourceFactory(source_name=PRESSHERALD_SOURCE)
        self.spider = MindenPressHeraldScrapyRssSpider()

    def test_parse_item_path(self):
        with open(join(dirname(__file__), 'files', 'pressherald.xml'), 'r') as f:
            file_content = f.read()
        feed_url = 'https://press-herald.com/feed/'
        ...
        assert self.articles[0]['title'].strip() == 'LAIB President speaks to Chamber about upcoming Nov. 13 Amendments'

        assert self.articles[0]['link'].strip() == 'https://press-herald.com/laib-president-speaks-to-chamber-about' \
                                                   '-upcoming-nov-13-amendments/'

        assert self.articles[0]['guid'].strip() == 'https://press-herald.com/?p=59342'

        assert self.articles[0]['published_date'].strip() == 'Wed, 20 Oct 2021 20:35:03 +0000'

    @patch('news_articles.spiders.pressherald_scrapy_rss.ItemLoader')
    @patch('news_articles.spiders.pressherald_scrapy_rss.RSSItem')
    ...
    @patch('news_articles.spiders.pressherald_scrapy_rss.ArticlePdfCreator')
    def test_parse_article(self, mock_article_pdf_creator):
        ...
        mock_css.assert_called_with(".elementor-widget-container>p")
        ...
        assert new_article.source.source_name == 'pressherald'
        ...
        assert crawled_post.source.source_name == 'pressherald'
