...
from news_articles.constants import BOSSIERPRESS_SOURCE
from news_articles.factories import NewsArticleSourceFactory
from news_articles.models import NewsArticle, CrawledPost
from news_articles.spiders import BossierPressScrapyRssSpider
from officers.factories import OfficerFactory
from utils.constants import FILE_TYPES


class BossierPressScrapyRssSpiderTestCase(TestCase):
    @patch('news_articles.spiders.base_scrapy_rss.GoogleCloudService')
    def setUp(self, mock_gcloud_service):
        NewsArticleSourceFactory(source_name=BOSSIERPRESS_SOURCE)
        self.spider = BossierPressScrapyRssSpider()

    def test_parse_item_path(self):
        with open(join(dirname(__file__), 'files', 'bossierpress.xml'), 'r') as f:
            file_content = f.read()
        feed_url = 'https://bossierpress.com/feed/'
        ...
        assert self.articles[0]['title'] == 'Haughton Man Arrested for Sexual Battery'

        assert self.articles[0]['link'] == 'https://bossierpress.com/haughton-man-arrested-for-sexual-battery/'

        assert self.articles[0]['guid'] == 'https://bossierpress.com/?p=131092'

        assert self.articles[0]['published_date'] == 'Tue, 19 Oct 2021 00:58:44 +0000'

    @patch('news_articles.spiders.bossierpress_scrapy_rss.ItemLoader')
    @patch('news_articles.spiders.bossierpress_scrapy_rss.RSSItem')
    ...
    @patch('news_articles.spiders.bossierpress_scrapy_rss.ArticlePdfCreator')
    def test_parse_article(self, mock_article_pdf_creator):
        ...
        mock_css.assert_called_with(".td-post-content>:not(aside):not(nav):not(img):not(div)")
        ...
        assert new_article.source.source_name == 'bossierpress'
        ...
        assert crawled_post.source.source_name == 'bossierpress'
