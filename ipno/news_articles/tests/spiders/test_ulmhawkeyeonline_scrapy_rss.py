...
from news_articles.constants import ULMHAWKEYEONLINE_SOURCE
from news_articles.factories import NewsArticleSourceFactory
from news_articles.models import NewsArticle, CrawledPost
from news_articles.spiders import TheHawkeyeScrapyRssSpider
from officers.factories import OfficerFactory
from utils.constants import FILE_TYPES


class TheHawkeyeScrapyRssSpiderTestCase(TestCase):
    @patch('news_articles.spiders.base_scrapy_rss.GoogleCloudService')
    def setUp(self, mock_gcloud_service):
        NewsArticleSourceFactory(source_name=ULMHAWKEYEONLINE_SOURCE)
        self.spider = TheHawkeyeScrapyRssSpider()

    def test_parse_item_path(self):
        with open(join(dirname(__file__), 'files', 'ulmhawkeyeonline.xml'), 'r') as f:
            file_content = f.read()
        feed_url = 'https://ulmhawkeyeonline.com/feed/'
        ...
        assert self.articles[0]['title'].strip() == 'Breaking News'

        assert self.articles[0]['link'].strip() == 'https://ulmhawkeyeonline.com/28962/breaking-news/breaking-news/'

        assert self.articles[0]['guid'].strip() == 'https://ulmhawkeyeonline.com/?p=28962'

        assert self.articles[0]['published_date'].strip() == 'Tue, 12 Oct 2021 23:45:04 +0000'

    @patch('news_articles.spiders.ulmhawkeyeonline_scrapy_rss.ItemLoader')
    @patch('news_articles.spiders.ulmhawkeyeonline_scrapy_rss.RSSItem')
    ...
    @patch('news_articles.spiders.ulmhawkeyeonline_scrapy_rss.ArticlePdfCreator')
    def test_parse_article(self, mock_article_pdf_creator):
        ...
        assert new_article.source.source_name == 'ulmhawkeyeonline'
        ...
        assert crawled_post.source.source_name == 'ulmhawkeyeonline'
