...
class TheAcadianaAdvocateScrapyRssSpiderTestCase(TestCase):
    ...
    def test_parse_item_path(self):
        with open(join(dirname(__file__), 'files', 'theacadianaadvocate.xml'), 'r') as f:
            file_content = f.read()
        ...
        assert self.articles[0]['title'] == 'Erath memorial evokes memories of 8 storm victims, and one man\'s ' \
                                            'promise to honor them'

        assert self.articles[0]['link'] == 'https://www.theadvocate.com/acadiana/article_a06ad9fa-2e3b-11ec-9f4b' \
                                           '-6714f5d20634.html'

        assert self.articles[0]['guid'].strip() == 'http://www.theadvocate.com/tncms/asset/editorial/a06ad9fa-2e3b' \
