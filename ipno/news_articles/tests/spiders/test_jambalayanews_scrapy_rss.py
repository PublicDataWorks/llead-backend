...
from news_articles.constants import JAMBALAYANEWS_SOURCE
from news_articles.factories import NewsArticleSourceFactory
from news_articles.models import NewsArticle, CrawledPost
from news_articles.spiders import JambalayaNewsScrapyRssSpider
from officers.factories import OfficerFactory
from utils.constants import FILE_TYPES


class JambalayaNewsScrapyRssSpiderTestCase(TestCase):
    @patch('news_articles.spiders.base_scrapy_rss.GoogleCloudService')
    def setUp(self, mock_gcloud_service):
        NewsArticleSourceFactory(source_name=JAMBALAYANEWS_SOURCE)
        self.spider = JambalayaNewsScrapyRssSpider()

    def test_parse_item_path(self):
        with open(join(dirname(__file__), 'files', 'jambalayanews.xml'), 'r') as f:
            file_content = f.read()
        feed_url = 'https://www.jambalayanews.com/search/?f=rss&t=article&l=50&s=start_time&sd=desc'
        ...
        assert self.articles[0]['title'].strip() == 'Kate Beckinsale: I have a very high IQ'

        assert self.articles[0]['link'].strip() == 'https://www.jambalayanews.com/lifestyles/entertainment/kate' \
                                                   '-beckinsale-i-have-a-very-high-iq/article_d42d4c32-7743-5c3d-94a4' \
                                                   '-4086ddf43dc5.html'

        assert self.articles[0]['guid'].strip() == 'http://www.jambalayanews.com/tncms/asset/editorial/d42d4c32-7743' \
                                                   '-5c3d-94a4-4086ddf43dc5'

        assert self.articles[0]['published_date'].strip() == 'Thu, 21 Oct 2021 20:00:00 -0500'

    @patch('news_articles.spiders.jambalayanews_scrapy_rss.ItemLoader')
    @patch('news_articles.spiders.jambalayanews_scrapy_rss.RSSItem')
    ...
    @patch('news_articles.spiders.jambalayanews_scrapy_rss.ArticlePdfCreator')
    def test_parse_article(self, mock_article_pdf_creator):
        ...
        mock_css.assert_called_with(
            "div[itemprop=\"articleBody\"]>p, div[itemprop=\"articleBody\"] div[dir=\"auto\"], "
            ".RichTextArticleBody-body, div[itemprop=\"articleBody\"]>div>p"
        )
        ...
        assert new_article.source.source_name == 'jambalayanews'
        ...
        assert crawled_post.source.source_name == 'jambalayanews'
