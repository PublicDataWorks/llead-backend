...
class RustonDailyLeaderScrapyRssSpiderTestCase(TestCase):
    ...
    def test_parse_item_path(self):
        ...
        assert self.articles[0]['title'].strip() == 'Should Gov. Edwards reinstate a mask mandate?'

        assert self.articles[0]['link'].strip() == 'http://www.rustonleader.com/content/should-gov-edwards-reinstate' \
                                                   '-mask-mandate'

        assert self.articles[0]['guid'].strip() == '95431 at http://www.rustonleader.com'

        assert self.articles[0]['published_date'].strip() == 'Mon, 02 Aug 2021 15:04:18 +0000'
