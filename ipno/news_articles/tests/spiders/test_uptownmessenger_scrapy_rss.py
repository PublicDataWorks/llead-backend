...
from news_articles.constants import UPTOWNMESSENGER_SOURCE
from news_articles.factories import NewsArticleSourceFactory
from news_articles.models import NewsArticle, CrawledPost
from news_articles.spiders import UptownMessengerScrapyRssSpider
...
class RustonDailyLeaderScrapyRssSpiderTestCase(TestCase):
    ...
    def setUp(self, mock_gcloud_service):
        NewsArticleSourceFactory(source_name=UPTOWNMESSENGER_SOURCE)
        self.spider = UptownMessengerScrapyRssSpider()

    def test_parse_item_path(self):
        with open(join(dirname(__file__), 'files', 'uptownmessenger.xml'), 'r') as f:
            file_content = f.read()
        feed_url = 'https://uptownmessenger.com/feed/'
        ...
        assert self.articles[0]['title'].strip() == 'A Letter from LaToya Cantrell (sponsored)'

        assert self.articles[0]['link'].strip() == 'https://uptownmessenger.com/2021/10/a-letter-from-latoya-cantrell' \
                                                   '-sponsored/'

        assert self.articles[0]['guid'].strip() == 'https://uptownmessenger.com/?p=626533'

        assert self.articles[0]['published_date'].strip() == 'Thu, 21 Oct 2021 16:45:23 +0000'

    @patch('news_articles.spiders.uptownmessenger_scrapy_rss.ItemLoader')
    @patch('news_articles.spiders.uptownmessenger_scrapy_rss.RSSItem')
    ...
    @patch('news_articles.spiders.uptownmessenger_scrapy_rss.ArticlePdfCreator')
    def test_parse_article(self, mock_article_pdf_creator):
        ...
        expected_css_calls = [
            call("span[itemprop=\"author\"]"),
            call("div[itemprop=\"articleBody\"]>p"),
        ]
        ...
        assert new_article.source.source_name == 'uptownmessenger'
        ...
        assert crawled_post.source.source_name == 'uptownmessenger'
