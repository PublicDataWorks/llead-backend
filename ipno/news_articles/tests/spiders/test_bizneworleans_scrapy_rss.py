...
class BizNewOrleansScrapyRssSpiderTestCase(TestCase):
    ...
    def test_parse_item_path(self):
        ...
        assert (
            ...
            == "LCMC Health Launches Second Part Of Its $7 Million Storm Relief Funds"
        )

        assert (
            self.articles[0]["link"].strip()
            == "https://www.bizneworleans.com/lcmc-health-launches-second-part-of"
            "-its-7-million-storm-relief-funds/"
        )

        assert (
            self.articles[0]["guid"].strip()
            == "https://www.bizneworleans.com/?p=269378"
        )

        assert (
            self.articles[0]["published_date"].strip()
            == "Wed, 20 Oct 2021 19:35:19 +0000"
        )
