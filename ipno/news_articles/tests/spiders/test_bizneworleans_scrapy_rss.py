...
from news_articles.constants import BIZNEWORLEANS_SOURCE
from news_articles.factories import NewsArticleSourceFactory
from news_articles.models import NewsArticle, CrawledPost
from news_articles.spiders import BizNewOrleansScrapyRssSpider
from officers.factories import OfficerFactory
from utils.constants import FILE_TYPES


class BizNewOrleansScrapyRssSpiderTestCase(TestCase):
    @patch('news_articles.spiders.base_scrapy_rss.GoogleCloudService')
    def setUp(self, mock_gcloud_service):
        NewsArticleSourceFactory(source_name=BIZNEWORLEANS_SOURCE)
        self.spider = BizNewOrleansScrapyRssSpider()

    def test_parse_item_path(self):
        with open(join(dirname(__file__), 'files', 'bizneworleans.xml'), 'r') as f:
            file_content = f.read()
        feed_url = 'https://www.bizneworleans.com/feed/'
        ...
        assert self.articles[0]['title'].strip() == 'LCMC Health Launches Second Part Of Its $7 Million Storm Relief ' \
                                                    'Funds'

        assert self.articles[0]['link'].strip() == 'https://www.bizneworleans.com/lcmc-health-launches-second-part-of' \
                                                   '-its-7-million-storm-relief-funds/'

        assert self.articles[0]['guid'].strip() == 'https://www.bizneworleans.com/?p=269378'

        assert self.articles[0]['published_date'].strip() == 'Wed, 20 Oct 2021 19:35:19 +0000'

        assert self.articles[0]['author'] == 'Site Staff'

        assert self.articles[0]['content'][:20] == '<div><p><img class=\"'
        assert self.articles[0]['content'][-20:] == 'ew Orleans</a>.</p>\n'

    @patch('news_articles.spiders.bizneworleans_scrapy_rss.ItemLoader')
    @patch('news_articles.spiders.bizneworleans_scrapy_rss.RSSItem')
    ...
    @patch('news_articles.spiders.bizneworleans_scrapy_rss.ArticlePdfCreator')
    def test_create_article(self, mock_article_pdf_creator):
        ...
        assert new_article.source.source_name == 'bizneworleans'
        ...
        assert crawled_post.source.source_name == 'bizneworleans'
