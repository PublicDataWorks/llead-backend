...
from news_articles.constants import MYARKLAMISS_SOURCE
from news_articles.factories import NewsArticleSourceFactory
from news_articles.models import NewsArticle, CrawledPost
from news_articles.spiders import MyArkLamissScrapyRssSpider
from officers.factories import OfficerFactory
from utils.constants import FILE_TYPES


class MyArkLamissScrapyRssSpiderTestCase(TestCase):
    @patch('news_articles.spiders.base_scrapy_rss.GoogleCloudService')
    def setUp(self, mock_gcloud_service):
        NewsArticleSourceFactory(source_name=MYARKLAMISS_SOURCE)
        self.spider = MyArkLamissScrapyRssSpider()

    def test_parse_item_path(self):
        with open(join(dirname(__file__), 'files', 'myarklamiss.xml'), 'r') as f:
            file_content = f.read()
        feed_url = 'https://www.myarklamiss.com/feed/'
        ...
        assert self.articles[0]['title'].strip() == 'Four men arrested for weapon and drug charges on Grambling campus'

        assert self.articles[0]['link'].strip() == 'https://www.myarklamiss.com/news/top-stories/four-men-arrested' \
                                                   '-for-weapon-and-drug-charges-on-grambling-campus/'

        assert self.articles[0]['guid'].strip() == 'https://www.myarklamiss.com/?p=951357'

        assert self.articles[0]['published_date'].strip() == 'Wed, 20 Oct 2021 15:48:32 +0000'

    @patch('news_articles.spiders.myarklamiss_scrapy_rss.ItemLoader')
    @patch('news_articles.spiders.myarklamiss_scrapy_rss.RSSItem')
    ...
    @patch('news_articles.spiders.myarklamiss_scrapy_rss.ArticlePdfCreator')
    def test_parse_article(self, mock_article_pdf_creator):
        ...
        assert new_article.source.source_name == 'myarklamiss'
        ...
        assert crawled_post.source.source_name == 'myarklamiss'
