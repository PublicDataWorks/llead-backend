...
from news_articles.constants import WGNO_SOURCE
from news_articles.factories import NewsArticleSourceFactory
from news_articles.models import NewsArticle, CrawledPost
from news_articles.spiders import WGNOScrapyRssSpider
from officers.factories import OfficerFactory
from utils.constants import FILE_TYPES


class WGNOScrapyRssSpiderTestCase(TestCase):
    @patch('news_articles.spiders.base_scrapy_rss.GoogleCloudService')
    def setUp(self, mock_gcloud_service):
        NewsArticleSourceFactory(source_name=WGNO_SOURCE)
        self.spider = WGNOScrapyRssSpider()

    def test_parse_item_path(self):
        with open(join(dirname(__file__), 'files', 'wgno.xml'), 'r') as f:
            file_content = f.read()
        feed_url = 'https://wgno.com/feed/'
        ...
        assert self.articles[0]['title'].strip() == 'New emergency protocols approved for New Orleans senior living ' \
                                                    'facilities'

        assert self.articles[0]['link'].strip() == 'https://wgno.com/hurricane-ida/new-emergency-protocols-approved' \
                                                   '-for-new-orleans-senior-living-facilities/'

        assert self.articles[0]['guid'].strip() == 'https://wgno.com/?p=858974'

        assert self.articles[0]['published_date'].strip() == 'Fri, 22 Oct 2021 03:08:00 +0000'

        assert self.articles[0]['content'][:20] == '\n<p>NEW ORLEANS (<a '
        assert self.articles[0]['content'][-20:] == 'ricane season. </p>\n'

    @patch('news_articles.spiders.wgno_scrapy_rss.ItemLoader')
    @patch('news_articles.spiders.wgno_scrapy_rss.RSSItem')
    ...
    @patch('news_articles.spiders.wgno_scrapy_rss.ArticlePdfCreator')
    def test_create_article(self, mock_article_pdf_creator):
        ...
        assert new_article.source.source_name == 'wgno'
        ...
        assert crawled_post.source.source_name == 'wgno'
