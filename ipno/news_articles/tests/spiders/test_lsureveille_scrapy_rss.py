...
from news_articles.spiders import ReveilleScrapyRssSpider
from officers.factories import OfficerFactory
from utils.constants import FILE_TYPES


class ReveilleScrapyRssSpiderTestCase(TestCase):
    @patch('news_articles.spiders.base_scrapy_rss.GoogleCloudService')
    def setUp(self, mock_gcloud_service):
        NewsArticleSourceFactory(source_name=LSUREVEILLE_SOURCE)
        self.spider = ReveilleScrapyRssSpider()

    def test_parse_item_path(self):
        with open(join(dirname(__file__), 'files', 'lsureveille.xml'), 'r') as f:
            file_content = f.read()
        feed_url = 'https://www.lsureveille.com/search/?f=rss&t=article&l=100&s=start_time&sd=desc&k%5B%5D=%23topstory'
        ...
        assert self.articles[0]['title'].strip() == 'LSU to disclose COVID-19 cases to public, reports 22 positive ' \
                                                    'cases in four days'

        assert self.articles[0]['link'].strip() == 'https://www.lsureveille.com/coronavirus/lsu-to-disclose-covid-19' \
                                                   '-cases-to-public-reports-22-positive-cases-in-four-days' \
                                                   '/article_76dd2638-e274-11ea-b1ac-7f51ef46b62d.html'

        assert self.articles[0]['guid'].strip() == 'http://www.lsureveille.com/tncms/asset/editorial/76dd2638-e274' \
                                                   '-11ea-b1ac-7f51ef46b62d'

        assert self.articles[0]['published_date'] == 'Wed, 19 Aug 2020 18:34:00 -0500'

    @patch('news_articles.spiders.lsureveille_scrapy_rss.ItemLoader')
    @patch('news_articles.spiders.lsureveille_scrapy_rss.RSSItem')
    ...
    @patch('news_articles.spiders.lsureveille_scrapy_rss.ArticlePdfCreator')
    def test_parse_article(self, mock_article_pdf_creator):
        ...
        assert new_article.source.source_name == 'lsureveille'
        ...
        assert crawled_post.source.source_name == 'lsureveille'
