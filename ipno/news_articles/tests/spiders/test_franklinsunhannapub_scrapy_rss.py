...
from news_articles.constants import FRANKLINSUNHANNAPUB_SOURCE
from news_articles.factories import NewsArticleSourceFactory
from news_articles.models import NewsArticle, CrawledPost
from news_articles.spiders import TheFranklinSunScrapyRssSpider
from officers.factories import OfficerFactory
from utils.constants import FILE_TYPES


class TheFranklinSunScrapyRssSpiderTestCase(TestCase):
    @patch('news_articles.spiders.base_scrapy_rss.GoogleCloudService')
    def setUp(self, mock_gcloud_service):
        NewsArticleSourceFactory(source_name=FRANKLINSUNHANNAPUB_SOURCE)
        self.spider = TheFranklinSunScrapyRssSpider()

    def test_parse_item_path(self):
        with open(join(dirname(__file__), 'files', 'franklinsunhannapub.xml'), 'r') as f:
            file_content = f.read()
        feed_url = 'https://www.hannapub.com/search/?f=rss&t=article&c=franklinsun&l=100&s=start_time&sd=desc'
        ...
        assert self.articles[0]['title'].strip() == 'Patriots last home game of season dedicated to Veterans'

        assert self.articles[0]['link'].strip() == 'https://www.hannapub.com/franklinsun/patriots-last-home-game-of' \
                                                   '-season-dedicated-to-veterans/article_fa62c974-1e09-11eb-88a5' \
                                                   '-eb76bcd86308.html'

        assert self.articles[0]['guid'].strip() == 'http://www.hannapub.com/tncms/asset/editorial/fa62c974-1e09-11eb' \
                                                   '-88a5-eb76bcd86308'

        assert self.articles[0]['published_date'] == 'Wed, 04 Nov 2020 06:00:00 -0600'

    @patch('news_articles.spiders.franklinsunhannapub_scrapy_rss.ItemLoader')
    @patch('news_articles.spiders.franklinsunhannapub_scrapy_rss.RSSItem')
    ...
    @patch('news_articles.spiders.franklinsunhannapub_scrapy_rss.ArticlePdfCreator')
    def test_parse_article(self, mock_article_pdf_creator):
        ...
        assert new_article.source.source_name == 'franklinsunhannapub'
        ...
        assert crawled_post.source.source_name == 'franklinsunhannapub'
