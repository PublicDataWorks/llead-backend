...
class BRProudScrapyRssSpiderTestCase(TestCase):
    ...
    def test_parse_item_path(self):
        ...
        assert self.articles[1]['title'].strip() == 'East Ascension Drainage meeting becomes heated, board member ' \
                                                    'walked out by police'

        assert self.articles[1]['link'].strip() == 'https://www.brproud.com/news/east-ascension-drainage-meeting' \
                                                   '-becomes-heated-board-member-walked-out-by-police/'

        assert self.articles[1]['guid'].strip() == 'https://www.brproud.com/?p=1094209'

        assert self.articles[1]['published_date'].strip() == 'Tue, 19 Oct 2021 02:42:30 +0000'
