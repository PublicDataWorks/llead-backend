...
from news_articles.constants import NATCHITOCHESTIMES_SOURCE
from news_articles.factories import NewsArticleSourceFactory
from news_articles.models import NewsArticle, CrawledPost
from news_articles.spiders import NatchiochesTimesScrapyRssSpider
from officers.factories import OfficerFactory
from utils.constants import FILE_TYPES


class NatchiochesTimesScrapyRssSpiderTestCase(TestCase):
    @patch('news_articles.spiders.base_scrapy_rss.GoogleCloudService')
    def setUp(self, mock_gcloud_service):
        NewsArticleSourceFactory(source_name=NATCHITOCHESTIMES_SOURCE)
        self.spider = NatchiochesTimesScrapyRssSpider()

    def test_parse_item_path(self):
        with open(join(dirname(__file__), 'files', 'natchitochestimes.xml'), 'r') as f:
            file_content = f.read()
        feed_url = 'https://www.natchitochestimes.com/feed/'
        ...
        assert self.articles[0]['title'].strip() == 'NPSO arrests for Sept. 27-Oct. 10'

        assert self.articles[0]['link'].strip() == 'https://www.natchitochestimes.com/2021/10/21/npso-arrests-for' \
                                                   '-sept-27-oct-10/'

        assert self.articles[0]['guid'].strip() == 'https://www.natchitochestimes.com/?p=48810'

        assert self.articles[0]['published_date'].strip() == 'Thu, 21 Oct 2021 06:59:06 +0000'

        assert self.articles[0]['content'][:20] == '<figure id="attachme'
        assert self.articles[0]['content'][-20:] == 'c abuse battery</p>\n'

    @patch('news_articles.spiders.natchitochestimes_scrapy_rss.ItemLoader')
    @patch('news_articles.spiders.natchitochestimes_scrapy_rss.RSSItem')
    ...
    @patch('news_articles.spiders.natchitochestimes_scrapy_rss.ArticlePdfCreator')
    def test_create_article(self, mock_article_pdf_creator):
        ...
        assert new_article.source.source_name == 'natchitochestimes'
        ...
        assert crawled_post.source.source_name == 'natchitochestimes'
