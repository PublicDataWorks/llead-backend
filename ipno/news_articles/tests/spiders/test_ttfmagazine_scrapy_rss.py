...
from news_articles.spiders import TtfMagazineScrapyRssSpider
from officers.factories import OfficerFactory
from utils.constants import FILE_TYPES


class CTtfMagazineScrapyRssSpiderTestCase(TestCase):
    @patch('news_articles.spiders.base_scrapy_rss.GoogleCloudService')
    def setUp(self, mock_gcloud_service):
        NewsArticleSourceFactory(source_name=TTFMAGAZINE_SOURCE)
        self.spider = TtfMagazineScrapyRssSpider()

    def test_parse_item_path(self):
        with open(join(dirname(__file__), 'files', 'ttfmagazine.xml'), 'r') as f:
            file_content = f.read()
        feed_url = 'https://www.225batonrouge.com/feed'
        ...
        assert self.articles[0]['title'].strip() == 'BREC partners with esports company for events, new revenue stream'

        assert self.articles[0]['link'].strip() == 'https://www.225batonrouge.com/our-city/brec-partners-esports' \
                                                   '-company-events-new-revenue-stream'

        assert self.articles[0]['guid'].strip() == 'http://www.225batonrouge.com/?p=165327'

        assert self.articles[0]['published_date'].strip() == 'Fri, 15 Oct 2021 13:24:36 +0000'

        assert self.articles[0]['author'] == 'Jennifer Tormo'

        assert self.articles[0]['content'][:20] == '<p><strong>Expect vi'
        assert self.articles[0]['content'][-20:] == 'com">[225]</a>.</p>\n'

    @patch('news_articles.spiders.ttfmagazine_scrapy_rss.ItemLoader')
    @patch('news_articles.spiders.ttfmagazine_scrapy_rss.RSSItem')
    ...
    @patch('news_articles.spiders.ttfmagazine_scrapy_rss.ArticlePdfCreator')
    def test_create_article(self, mock_article_pdf_creator):
        ...
        assert new_article.source.source_name == 'ttfmagazine'
        ...
        assert crawled_post.source.source_name == 'ttfmagazine'
