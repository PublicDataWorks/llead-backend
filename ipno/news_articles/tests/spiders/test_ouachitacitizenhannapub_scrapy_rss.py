...
from news_articles.constants import OUACHITACITIZENHANNAPUB_SOURCE
from news_articles.factories import NewsArticleSourceFactory
from news_articles.models import NewsArticle, CrawledPost
from news_articles.spiders import TheOouachitaCitizenScrapyRssSpider
from officers.factories import OfficerFactory
from utils.constants import FILE_TYPES


class TheOouachitaCitizenScrapyRssSpiderTestCase(TestCase):
    @patch('news_articles.spiders.base_scrapy_rss.GoogleCloudService')
    def setUp(self, mock_gcloud_service):
        NewsArticleSourceFactory(source_name=OUACHITACITIZENHANNAPUB_SOURCE)
        self.spider = TheOouachitaCitizenScrapyRssSpider()

    def test_parse_item_path(self):
        with open(join(dirname(__file__), 'files', 'ouachitacitizenhannapub.xml'), 'r') as f:
            file_content = f.read()
        feed_url = 'https://www.hannapub.com/search/?f=rss&t=article&c=ouachitacitizen&l=100&s=start_time&sd=desc'
        ...
        assert self.articles[0]['title'].strip() == 'Coaches preview stacked Bayou Jamb event'

        assert self.articles[0]['link'].strip() == 'https://www.hannapub.com/ouachitacitizen/sports/coaches-preview' \
                                                   '-stacked-bayou-jamb-event/article_ff134658-0439-11ec-9652' \
                                                   '-b7caffe97ad6.html'

        assert self.articles[0]['guid'].strip() == 'http://www.hannapub.com/tncms/asset/editorial/ff134658-0439-11ec' \
                                                   '-9652-b7caffe97ad6'

        assert self.articles[0]['published_date'] == 'Mon, 23 Aug 2021 12:46:00 -0500'

    @patch('news_articles.spiders.ouachitacitizenhannapub_scrapy_rss.ItemLoader')
    @patch('news_articles.spiders.ouachitacitizenhannapub_scrapy_rss.RSSItem')
    ...
    @patch('news_articles.spiders.ouachitacitizenhannapub_scrapy_rss.ArticlePdfCreator')
    def test_parse_article(self, mock_article_pdf_creator):
        ...
        assert new_article.source.source_name == 'ouachitacitizenhannapub'
        ...
        assert crawled_post.source.source_name == 'ouachitacitizenhannapub'
