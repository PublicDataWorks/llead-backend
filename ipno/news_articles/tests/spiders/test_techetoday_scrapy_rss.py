...
from news_articles.constants import TECHETODAY_SOURCE
from news_articles.factories import NewsArticleSourceFactory
from news_articles.models import NewsArticle, CrawledPost
from news_articles.spiders import TecheTodayScrapyRssSpider
from officers.factories import OfficerFactory
from utils.constants import FILE_TYPES


class TecheTodayScrapyRssSpiderTestCase(TestCase):
    @patch('news_articles.spiders.base_scrapy_rss.GoogleCloudService')
    def setUp(self, mock_gcloud_service):
        NewsArticleSourceFactory(source_name=TECHETODAY_SOURCE)
        self.spider = TecheTodayScrapyRssSpider()

    def test_parse_item_path(self):
        with open(join(dirname(__file__), 'files', 'techetoday.xml'), 'r') as f:
            file_content = f.read()
        feed_url = 'https://www.techetoday.com/rss.xml'
        ...
        assert self.articles[0]['title'].strip() == 'Prevention and assistance tips as health care fraud grows ' \
                                                    'nationwide'

        assert self.articles[0]['link'].strip() == 'https://www.techetoday.com/news/prevention-and-assistance-tips' \
                                                   '-health-care-fraud-grows-nationwide'

        assert self.articles[0]['guid'].strip() == '197919 at https://www.techetoday.com'

        assert self.articles[0]['published_date'].strip() == 'Wed, 20 Oct 2021 06:00:00 +0000'

    @patch('news_articles.spiders.techetoday_scrapy_rss.ItemLoader')
    @patch('news_articles.spiders.techetoday_scrapy_rss.RSSItem')
    ...
    @patch('news_articles.spiders.techetoday_scrapy_rss.ArticlePdfCreator')
    def test_parse_article(self, mock_article_pdf_creator):
        ...
        mock_css.assert_called_with("div[property=\"content:encoded\"]>p")
        ...
        assert new_article.source.source_name == 'techetoday'
        ...
        assert crawled_post.source.source_name == 'techetoday'
