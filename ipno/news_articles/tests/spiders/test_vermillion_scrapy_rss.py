...
class VermillionTodayRssSpiderTestCase(TestCase):
    ...
    def test_parse_item_path(self):
        ...
        assert (
            ...
            == "Don Chauvin passes away after helping induce longtime friend into "
            "Abbeville High Hall of Fame"
        )

        assert (
            self.articles[0]["link"].strip()
            == "https://www.vermiliontoday.com/news/don-chauvin-passes-away-after-helping"
            "-induce-longtime-friend-abbeville-high-hall-fame"
        )

        assert (
            self.articles[0]["guid"].strip()
            == "199361 at https://www.vermiliontoday.com"
        )

        assert (
            self.articles[0]["published_date"].strip()
            == "Fri, 15 Oct 2021 15:11:37 +0000"
        )
