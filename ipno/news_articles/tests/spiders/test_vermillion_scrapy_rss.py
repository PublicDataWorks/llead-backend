...
class VermillionTodayRssSpiderTestCase(TestCase):
    @patch('news_articles.spiders.base_scrapy_rss.GoogleCloudService')
    def setUp(self, mock_gcloud_service):
        NewsArticleSourceFactory(source_name=VERMILIONTODAY_SOURCE)
        self.spider = VermillionTodayScrapyRssSpider()

    def test_parse_item_path(self):
        with open(join(dirname(__file__), 'files', 'vermillion.xml'), 'r') as f:
            file_content = f.read()
        feed_url = 'https://www.vermiliontoday.com/rss.xml'
        ...
        assert self.articles[0]['title'].strip() == 'Don Chauvin passes away after helping induce longtime friend into ' \
                                                    'Abbeville High Hall of Fame'

        assert self.articles[0][
                   'link'].strip() == 'https://www.vermiliontoday.com/news/don-chauvin-passes-away-after-helping' \
                                      '-induce-longtime-friend-abbeville-high-hall-fame'

        assert self.articles[0]['guid'].strip() == '199361 at https://www.vermiliontoday.com'

        assert self.articles[0]['published_date'].strip() == 'Fri, 15 Oct 2021 15:11:37 +0000'

    @patch('news_articles.spiders.vermilliontoday_scrapy_rss.ItemLoader')
    @patch('news_articles.spiders.vermilliontoday_scrapy_rss.RSSItem')
    ...
    @patch('news_articles.spiders.vermilliontoday_scrapy_rss.ArticlePdfCreator')
    def test_parse_article(self, mock_article_pdf_creator):
        ...
        mock_css.assert_called_with(".field-item.even>:not(meta):not(div):not(img)")
        ...
        assert new_article.source.source_name == 'vermiliontoday'
        ...
        assert crawled_post.source.source_name == 'vermiliontoday'
