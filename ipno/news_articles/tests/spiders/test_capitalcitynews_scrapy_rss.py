...
from news_articles.constants import CAPITALCITYNEWS_SOURCE
...
class CapitalCityNewsScrapyRssSpiderTestCase(TestCase):
    @patch('news_articles.spiders.base_scrapy_rss.GoogleCloudService')
    def setUp(self, mock_gcloud_service):
        NewsArticleSourceFactory(source_name=CAPITALCITYNEWS_SOURCE)
        self.spider = CapitalCityNewsScrapyRssSpider()

    def test_parse_item_path(self):
        with open(join(dirname(__file__), 'files', 'capitalcitynews.xml'), 'r') as f:
            file_content = f.read()
        feed_url = 'http://capitalcitynews.us/?feed=rss2'
        ...
        assert self.articles[0]['title'].strip() == 'Pastor’s Legal Battles Look Promising After Landmark Supreme Court Decision in NYC'

        assert self.articles[0]['link'].strip() == 'http://capitalcitynews.us/?p=3799'

        assert self.articles[0]['guid'].strip() == 'http://capitalcitynews.us/?p=3799'

        assert self.articles[0]['published_date'].strip() == 'Fri, 22 Jan 2021 22:40:29 +0000'

        assert self.articles[0]['author'] == 'admccn'

        assert self.articles[0]['content'][:20] == '<p>Legal problems fa'
        assert self.articles[0]['content'][-20:] == ' in prevailing.</p>\n'

    @patch('news_articles.spiders.capitalcitynews_scrapy_rss.ItemLoader')
    @patch('news_articles.spiders.capitalcitynews_scrapy_rss.RSSItem')
    ...
    @patch('news_articles.spiders.capitalcitynews_scrapy_rss.ArticlePdfCreator')
    def test_create_article(self, mock_article_pdf_creator):
        ...
        assert new_article.source.source_name == 'capitalcitynews'
        ...
        assert crawled_post.source.source_name == 'capitalcitynews'
