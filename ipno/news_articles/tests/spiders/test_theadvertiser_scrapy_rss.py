...
class DailyAdvertiserScrapyRssSpiderTestCase(TestCase):
    @patch('news_articles.spiders.base_scrapy_rss.GoogleCloudService')
    def setUp(self, mock_gcloud_service):
        NewsArticleSourceFactory(source_name=THEADVERTISER_SOURCE)
        self.spider = DailyAdvertiserScrapyRssSpider()

    def test_parse_item_path(self):
        with open(join(dirname(__file__), 'files', 'theadvertiser.xml'), 'r') as f:
            file_content = f.read()
        feed_url = 'http://rssfeeds.theadvertiser.com/lafeyettela/news'
        ...
        assert self.articles[0]['title'].strip() == 'Lafayette middle-schooler \'gets curious\' with Vice ' \
                                                    'President Kamala Harris, NASA astronaut'

        assert self.articles[0]['link'].strip() == 'https://www.theadvertiser.com/story/news/2021/10/26/lafayette' \
                                                   '-middle-schooler-gets-curious-u-s-vice-president-nasa-astronaut' \
                                                   '/6135881001/'

        assert self.articles[0]['guid'].strip() == 'https://www.theadvertiser.com/story/news/2021/10/26/lafayette' \
                                                   '-middle-schooler-gets-curious-u-s-vice-president-nasa-astronaut' \
                                                   '/6135881001/'

        assert self.articles[0]['published_date'] == 'Tue, 26 Oct 2021 01:00:12 +0000'

    @patch('news_articles.spiders.theadvertiser_scrapy_rss.ItemLoader')
    @patch('news_articles.spiders.theadvertiser_scrapy_rss.RSSItem')
    ...
    def test_parse_article(self, mock_article_pdf_creator):
        ...
        assert new_article.source.source_name == 'theadvertiser'
    ...
    def test_parse_guid(self):
        guid = 'https://www.theadvertiser.com/story/news/2021/10/26/lafayette-middle-schooler-gets-curious-u-s-vice' \
               '-president-nasa-astronaut/6135881001/'
        result = self.spider.parse_guid(guid)

        assert result == 'lafayette-middle-schooler-gets-curious-u-s-vice-president-nasa-astronaut-6135881001'
