...
class ProcessRematchOfficersTestCase(TestCase):
    ...
    def test_commit_data_to_wrgl(self):
        ...
        assert self.pro.wrgl.create_wrgl_commit.call_args[0][0] == "+ 1 officer(s)"
        assert self.pro.wrgl.create_wrgl_commit.call_args[0][1] == [
            "uid",
            "newsarticle_id",
        ]
        assert (
            self.pro.wrgl.create_wrgl_commit.call_args[0][2].first().uid
            == data.first().uid
        )
        assert (
            self.pro.wrgl.create_wrgl_commit.call_args[0][3] == "news_article_officer"
        )
