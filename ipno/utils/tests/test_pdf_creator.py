...
class ArticlePdfCreatorTestCase(TestCase):
    def setUp(self):
        ...
        self.content = [
            ...
            {"style": "Heading4", "content": "This is subtitle"},
            {
                "style": "BodyText",
        ...
        self.pdf = ArticlePdfCreator(
            self.title, self.author, self.date, self.content, self.link
        )

    def test_add_page_number_fake(self):
        canvas = MagicMock()
        doc = MagicMock(page=1)
    ...
    def test_build_body(self, mock_paragraph):
        def mock_paragraph_side_effect(paragraph_text, paragraph_style):
            return {"text": paragraph_text, "style": paragraph_style}
        ...
        assert body == [
            ...
            {"text": "This is subtitle", "style": "Heading4"},
            {
    ...
    def test_build_pdf_flow(
        ...
        mock_simple_doc_template_object = Mock(multiBuild=mock_multi_build)
        ...
        def mock_get_style_side_effect(style_name, custom_style={}):
            custom_style_list = [
                f"{key}-{value}" for key, value in custom_style.items()
            ]
    ...
    def test_build_pdf_flow_without_author(
        ...
        self.pdf_without_author = ArticlePdfCreator(
            self.title, None, self.date, self.content, self.link
        )
