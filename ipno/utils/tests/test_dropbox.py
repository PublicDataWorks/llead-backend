...
class DropboxServiceTestCase(TestCase):
    ...
    def test_generate_new_dropbox_token_success(
        self, MockDropboxOAuth2FlowNoRedirect, MockInput
    ):
    ...
    def test_get_temporary_link_fail_due_to_missing_refresh_token(self):
        with raises(
            ValueError, match="No dropbox refresh token found, please init it first"
        ):
            DropboxService()
    ...
    def test_get_temporary_link_from_path_success(self, MockDropBox):
        ...
        mock_dropbox_client = MagicMock()
        mock_dropbox_client.check_and_refresh_access_token = (
            mock_check_and_refresh_access_token
        )
