...
from utils.cache_utils import delete_cache, custom_cache, flush_news_article_related_caches
...
class CacheUtilsTestCase(TestCase):
    ...
    def test_flush_news_article_related_caches_no_start_time(self):
        ...
        news_article = NewsArticleFactory(content='Text content', author='Writer Staff', source=source)
        ...
        cache.set(reverse('api:officers-timeline', kwargs={'pk': officer.id}), 'Timeline')
        cache.set(reverse('api:departments-detail', kwargs={'pk': department.slug}), 'Department detail')
        cache.set(reverse('api:departments-news-articles', kwargs={'pk': department.slug}), 'Department article')
        ...
        assert not cache.get(reverse('api:officers-timeline', kwargs={'pk': officer.id}))
        assert not cache.get(reverse('api:departments-detail', kwargs={'pk': department.slug}))
        assert not cache.get(reverse('api:departments-news-articles', kwargs={'pk': department.slug}))
    ...
    def test_flush_news_article_related_caches_with_start_time(self):
        ...
        cache.set(reverse('api:news-articles-list'), 'News article list')
        cache.set(reverse('api:analytics-summary'), 'Summary')
        cache.set(reverse('api:officers-timeline', kwargs={'pk': officer_1.id}), 'Timeline of officer 1')
        cache.set(reverse('api:officers-timeline', kwargs={'pk': officer_2.id}), 'Timeline of officer 2')
        cache.set(reverse('api:departments-detail', kwargs={'pk': department_1.slug}), 'Department 1 detail')
        cache.set(reverse('api:departments-detail', kwargs={'pk': department_2.slug}), 'Department 2 detail')
        cache.set(reverse('api:departments-news-articles', kwargs={'pk': department_1.slug}), 'Department 1 article')
        cache.set(reverse('api:departments-news-articles', kwargs={'pk': department_2.slug}), 'Department 1 article')
        cache.set(reverse('api:officers-list'), 'Officer list')

        flush_news_article_related_caches(datetime(2021, 9, 3, 9, 0, 0, tzinfo=pytz.utc))

        assert not cache.get(reverse('api:news-articles-list'))
        assert not cache.get(reverse('api:analytics-summary'))
        assert not cache.get(reverse('api:officers-timeline', kwargs={'pk': officer_2.id}))
        assert not cache.get(reverse('api:departments-detail', kwargs={'pk': department_2.slug}))
        assert not cache.get(reverse('api:departments-news-articles', kwargs={'pk': department_2.slug}))
        assert cache.get(reverse('api:officers-timeline', kwargs={'pk': officer_1.id}))
        assert cache.get(reverse('api:departments-detail', kwargs={'pk': department_1.slug}))
        assert cache.get(reverse('api:departments-news-articles', kwargs={'pk': department_1.slug}))
        assert cache.get(reverse('api:officers-list'))
