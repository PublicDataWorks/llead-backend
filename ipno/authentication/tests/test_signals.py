...
class AuthenticationTestCase(TestCase):
    ...
    def test_password_reset_token_created(self, mock_send_mail):
        ...
        message = reset_password_template.replace('{reset_password_token}', token).replace('{HOST}', settings.HOST)
        mock_send_mail.assert_called_with(
            subject="Password Reset",
            from_email=settings.FROM_EMAIL,
            recipient_list=[user.email],
            html_message=render_to_string("email/dynamic_email.html", {"message": message}),
