...
class OfficerDatafileQueryTestCase(TestCase):
    def test_generate_sheets(self):
        ...
        complaint_1 = ComplaintFactory(agency=department.agency_name)
        complaint_2 = ComplaintFactory(agency=department.agency_name)
        ...
        citizen = CitizenFactory(use_of_force=uof)
        ...
        expected_document = {
            **{
                key: getattr(document, key) if hasattr(document, key) else {}
                for key in OFFICER_DOC_FIELDS
            },
        }

        expected_event_1 = {
            **{
                key: getattr(event_1, key) if hasattr(event_1, key) else {}
        ...
        expected_event_2 = {
            **{
                key: getattr(event_2, key) if hasattr(event_2, key) else {}
                for key in OFFICER_INCIDENT_FIELDS
            },
            "uid": officer.uid,
            "agency": department.agency_name,
        ...
        expected_complaint_receive_event = {
            **{
                key: getattr(complaint_receive_event, key)
                if hasattr(complaint_receive_event, key)
                else {}
                for key in OFFICER_INCIDENT_FIELDS
            },
            "uid": officer.uid,
        ...
        expected_uof_receive_event = {
            **{
                key: getattr(uof_receive_event, key)
                if hasattr(uof_receive_event, key)
                else {}
                for key in OFFICER_INCIDENT_FIELDS
            },
            "uid": officer.uid,
        ...
        expected_complaint_1 = {
            **{
                key: getattr(complaint_1, key) if hasattr(complaint_1, key) else {}
                for key in OFFICER_COMPLAINT_FIELDS
            },
            "agency": department.agency_name,
        }
        expected_complaint_2 = {
            **{
                key: getattr(complaint_2, key) if hasattr(complaint_2, key) else {}
                for key in OFFICER_COMPLAINT_FIELDS
            },
            "agency": department.agency_name,
        }
        ...
        expected_uof = [
            {
                **{
                    key: getattr(uof, key) if hasattr(uof, key) else {}
                    for key in OFFICER_UOF_FIELDS
                },
            }
        ]
        ...
        expected_citizen = [
            {
                **{
                    key: getattr(citizen, key) if hasattr(citizen, key) else {}
                    for key in OFFICER_CITIZEN_FIELDS
                },
            }
        ]
        ...
        expected_citizen_sheet = pd.DataFrame(expected_citizen)
        expected_citizen_sheet.dropna(how="all", axis=1, inplace=True)
        expected_incident_sheet = pd.DataFrame(expected_incidents)
        expected_incident_sheet_sorted = expected_incident_sheet.sort_values(
            by=["event_uid"]
        ...
        expected_career_sheet_sorted = expected_career_sheet.sort_values(
            by=["event_uid"]
        ).reset_index(drop=True)
        expected_career_sheet_sorted.dropna(how="all", axis=1, inplace=True)
        expected_complaint_sheet = pd.DataFrame(expected_complaints)
        expected_complaint_sheet_sorted = expected_complaint_sheet.sort_values(
            by=["allegation_uid"]
        ).reset_index(drop=True)
        ...
        xlsx_officer_data = pd.read_excel(
            data_file, sheet_name=OFFICER_PROFILE_SHEET, dtype=str
        )
        xlsx_uof_data = pd.read_excel(
            data_file, sheet_name=OFFICER_UOF_SHEET, dtype=str
        )
        xlsx_citizen_data = pd.read_excel(data_file, sheet_name=OFFICER_CITIZEN_SHEET)
        xlsx_incident_data = pd.read_excel(data_file, sheet_name=OFFICER_INCIDENT_SHEET)
        xlsx_incident_data_sorted = xlsx_incident_data.sort_values(
            by=["event_uid"]
        ).reset_index(drop=True)
        xlsx_career_data = pd.read_excel(data_file, sheet_name=OFFICER_CAREER_SHEET)
        xlsx_career_data_sorted = xlsx_career_data.sort_values(
            by=["event_uid"]
        ).reset_index(drop=True)
        xlsx_complaint_data = pd.read_excel(
            data_file, sheet_name=OFFICER_COMPLAINT_SHEET
        )
        xlsx_complaint_data_sorted = xlsx_complaint_data.sort_values(
            by=["allegation_uid"]
        ).reset_index(drop=True)
        xlsx_doc_data = pd.read_excel(
            data_file, sheet_name=OFFICER_DOC_SHEET, dtype=str
        )
        ...
        expected_officer_sheet_strip = expected_officer_sheet.apply(
            lstrip0, axis="columns"
        )

        pd.testing.assert_frame_equal(xlsx_officer_data, expected_officer_sheet_strip)
        pd.testing.assert_frame_equal(
            xlsx_uof_data, expected_uof_sheet, check_like=True
        )
        pd.testing.assert_frame_equal(
            xlsx_citizen_data, expected_citizen_sheet, check_like=True
        )
        pd.testing.assert_frame_equal(
            xlsx_doc_data, expected_doc_sheet, check_like=True
        )

        pd.testing.assert_frame_equal(
            xlsx_incident_data_sorted,
            expected_incident_sheet_sorted,
            check_like=True,
            check_dtype=False,
        )
        pd.testing.assert_frame_equal(
            xlsx_career_data_sorted,
            expected_career_sheet_sorted,
            check_like=True,
            check_dtype=False,
        )
        pd.testing.assert_frame_equal(
            xlsx_complaint_data_sorted,
            expected_complaint_sheet_sorted,
            check_like=True,
            check_dtype=False,
        )
